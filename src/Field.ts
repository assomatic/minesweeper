...
export const fieldGenerator = (size: number, propabiltiy: number): Field => {
  if (propabiltiy < 0 || propabiltiy > 1) {
    throw new Error('Propabiltiy must be between 0 and 1 - fromFunction')
  }

  let unprocessedCells = size * size
  let restCellsWithBombs = unprocessedCells * propabiltiy
